...
export default function QueuePage() {
  ...
  return (
    <div className="qship-queue-page">
      <div className="qship-queue-hero">
        <div>
          <h2>Release approvals</h2>
          <p>
            Agent actions and ship steps wait here until you approve. Nothing deploys without your sign-off —
      ...
      <div className="qship-queue-tabs">
        <button
          type="button"
          className="qship-inbox-tab"
      ...
      <div className="qship-queue-search">
        <Search size={14} className="qship-queue-search-icon" />
        <input
      ...
      {itemsQuery.isLoading ? (
        ...
        <QueryErrorState
          ...
          className="qship-app-empty"
          style={{ marginTop: 24 }}
        />
      ) : tab === "pending" && pending.length === 0 ? (
        <div className="qship-app-empty" style={{ marginTop: 24 }}>
          <div className="qship-app-empty-icon">
            <Sparkles size={24} />
          </div>
          <div>
            <h3>Nothing waiting for approval</h3>
            <p>When the agent queues a PRD, review, or release step, it will show up here first.</p>
          </div>
          <Link
            href="/agent"
        ...
        <div className="qship-queue-list">
          {filtered.map((item) => {
            ...
            return (
              <article key={item.id} className="qship-queue-card" data-status={item.status}>
                <div className="qship-queue-card-head">
                  <span className="qship-queue-card-icon">
                    <Icon size={16} />
                  </span>
                  <div className="qship-queue-card-meta">
                    <h3>{item.title}</h3>
                    <div className="qship-queue-card-tags">
                      <span className="qship-mono-tag">{kindLabel(item.kind, item.payload)}</span>
                      <span className="qship-queue-card-time">
                        <Clock3 size={12} />
                  ...
                  {!isPending ? (
                    <span className="qship-queue-status" data-status={item.status}>
                ...
                {item.preview ? <p className="qship-queue-card-preview">{item.preview}</p> : null}
                {(item.kind === "email_send" || item.kind === "email_draft") &&
                  (item.payload?.cc || item.payload?.bcc) ? (
                  <div className="qship-queue-card-recipients" style={{ display: "flex", gap: 12, fontSize: 12, color: "var(--muted)", marginTop: 4 }}>
                ...
                {item.errorMessage ? (
                  <p className="qship-queue-card-error">{item.errorMessage}</p>
                ) : null}

                {isPending || canRetry ? (
                  <div className="qship-queue-card-actions">
                    <button
                      type="button"
                      className="qship-btn-ghost"
                      disabled={anyBusy}
      ...
      {archiveConfirm ? (
        ...
        >
          <div
            className="qship-modal qship-cal-delete-modal qship-cal-confirm-modal"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="qship-modal-head">
              <h3>Confirm dates</h3>
              <button
                type="button"
                className="qship-app-iconbtn"
                disabled={anyBusy}
            ...
            <div className="qship-cal-event-detail">
              <p className="qship-cal-confirm-title">{archiveConfirm.title}</p>
              <p className="qship-cal-event-detail-copy">
              ...
              <div className="qship-modal-row">
                <div>
                  <label className="qship-set-label" htmlFor="archive-start">
                    Starts
                  </label>
                  <input
                    id="archive-start"
                ...
                <div>
                  <label className="qship-set-label" htmlFor="archive-end">
                    Ends
                  </label>
                  <input
                    id="archive-end"
              ...
              {archiveDateError ? (
                <p className="qship-cal-date-error">{archiveDateError}</p>
              ) : null}
            </div>
