...
function zigzagAnchors(
  ...
  const source =
    anchorPoint(fromEl, ".qship-flow-anchor--out", container) ??
    (goingRight
      ? {
  ...
  const target =
    anchorPoint(toEl, ".qship-flow-anchor--in", container) ??
    (goingRight
      ? {
...
export function FlowDottedConnectors({
  ...
  return (
    <svg ref={svgRef} className="qship-flow-dotted-svg" aria-hidden>
      <defs>
        <marker
          id="qship-flow-arrow"
          viewBox="0 0 10 10"
          ...
          orient="auto"
        >
          <path d="M 1 1 L 9 5 L 1 9 Z" fill="var(--qship-accent-bright)" />
        </marker>
      </defs>
      {paths.map((path, i) => (
        <path
          ...
          d={path.d}
          fill="none"
          stroke="var(--qship-accent-bright)"
          strokeWidth={1.5}
          strokeDasharray="6 5"
          strokeLinecap="round"
          markerEnd={edgeVisible[i] ? "url(#qship-flow-arrow)" : undefined}
          style={{
