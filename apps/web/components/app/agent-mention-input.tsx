  ...
  return (
    <div className="qship-agent-composer-wrap">
      {syncPhase === "syncing" && syncProgress ? (
        <div className="qship-agent-sync-banner">
      ...
      {showSuggestions ? (
        <ul className="qship-agent-mentions" role="listbox" aria-label="Contact suggestions">
          {syncPhase === "syncing" || (search.isFetching && suggestions.length === 0) ? (
            <li className="qship-agent-mentions-empty">
              <Loader2 size={12} className="qship-spin" /> Loading senders…
            </li>
          ...
          {suggestions.map((contact, i) => (
            <li key={contact.id}>
              <button
                ...
                className="qship-agent-mentions-item"
                onMouseDown={(e) => e.preventDefault()}
                onClick={() => pickContact(contact)}
              >
                <UserRound size={14} />
                <span className="qship-agent-mentions-main">
      ...
      <div className="qship-agent-composer">
        <input
          ref={inputRef}
          value={value}
          onChange={(e) => {
