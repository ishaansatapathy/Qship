...
export function AgentContextPicker({ open, onClose, onSelect, disabled }: AgentContextPickerProps) {
  ...
  return (
    <div className="qship-agent-context-picker" role="dialog" aria-label="Attach context">
      ...
      <div className="qship-agent-context-picker-tabs" role="tablist">
        <button
          type="button"
          role="tab"
      ...
      <ul className="qship-agent-context-picker-list" role="listbox">
        {loading ? (
          <li className="qship-agent-context-picker-empty">
            <Loader2 size={14} className="qship-spin" />
            Loading…
          </li>
        ) : tab === "inbox" ? (
          threads.length === 0 ? (
            <li className="qship-agent-context-picker-empty">No threads found</li>
          ) : (
            threads.map((thread) => (
              <li key={thread.id}>
                <button
          ...
          <li className="qship-agent-context-picker-empty">No events found</li>
        ) : (
          events.map((event) => (
            <li key={event.id}>
              <button
