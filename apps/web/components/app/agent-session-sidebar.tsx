...
export function AgentSessionSidebar({
  ...
  return (
    <aside className="qship-agent-sidebar" aria-label="Agent conversations">
      <div className="qship-agent-sidebar-head">
        <MessagesSquare size={14} />
        Chats
        <button
          type="button"
          className="qship-agent-sidebar-new"
          onClick={onNewChat}
          disabled={disabled}
          title="New chat"
        >
        ...
        {sessionsQuery.isLoading ? (
          ...
          sessions.map((session) => {
            ...
            return (
              <div
                key={session.id}
                className="qship-agent-sidebar-item-wrap"
                data-active={active ? "true" : undefined}
              >
                <button
                  type="button"
                  className="qship-agent-sidebar-item"
                  onClick={() => onSelectSession(session.id)}
                  disabled={disabled}
                >
                  <span className="qship-agent-sidebar-item-title">{sessionTitle(session)}</span>
                  <span className="qship-agent-sidebar-item-meta">
                ...
                <button
                  type="button"
                  className="qship-agent-sidebar-delete"
