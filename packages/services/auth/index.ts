...
async function sendVerificationEmail(email: string, verificationToken: string) {
  ...
  await sendEmail({
    email,
    subject: "Verify your Qship account",
    html: `<p>Welcome to Qship! <a href="${verifyUrl}">Verify your email</a></p>`,
...
class AuthService {
  ...
  private async initiateTwoFactorChallenge(user: SelectUser) {
    ...
    await sendEmail({
      email: user.email,
      subject: "Your Qship verification code",
  ...
  public async forgotPassword(input: ForgotPasswordInput): Promise<{ message: string }> {
    ...
    if (input.method === "otp") {
      await sendEmail({
        email: user.email,
        subject: "Your Qship password reset code",
      ...
      await sendEmail({
        email: user.email,
        subject: "Reset your Qship password",
